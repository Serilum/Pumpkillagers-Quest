...
public class Scheduler {
            ...
            level.getServer().execute(() -> {
                ...
                if (character != null) {
                    if (!character.isRemoved()) {
                        Set<String> tags = character.entityTags();
                        ...
                        case "wear" -> {
                            if (character != null) {
                                if (messagePair.getSecond().getString().contains("freed me from these shackles")) {
                                    character.setVillagerData(character.getVillagerData().withType(level.registryAccess(), VillagerType.SNOW).withProfession(level.registryAccess(), VillagerProfession.WEAPONSMITH));
                        ...
                        case "drop" -> {
                            if (characterVec != null) {
                                ...
                                if (itemStack.getItem().equals(Items.PAPER)) {
                                    Player player = messagePair.getFirst();
                                    player.entityTags().add(Reference.MOD_ID + ".unleashed");
                                }
                            }
                        }
                    }
    ...
    public static void scheduleCharacterLeave(Level level, Villager character, int msDelay) {
        new Thread(() -> {
            ...
            character.entityTags().remove(Reference.MOD_ID + ".persistent");
