...
public class Data {
    ...
    public static final TagKey<Block> pumpkinTag = TagKey.create(Registries.BLOCK, ResourceLocation.fromNamespaceAndPath("minecraft", "crops/pumpkin"));
    ...
    public static ItemStack getQuestbook(Level level) {
        ...
        try {
            ...
            ItemStack questBookStack = ItemStack.CODEC.parse(level.registryAccess().createSerializationContext(NbtOps.INSTANCE), compoundTag).result().orElse(ItemStack.EMPTY);
            if (!questBookStack.isEmpty()) {
                questBookStack.set(DataComponents.CUSTOM_NAME, Component.translatable(questBookName).withStyle(ChatFormatting.YELLOW));
    ...
    public static ItemStack getStopPkbook(Level level) {
        ...
        try {
            ...
            ItemStack questBookStack = ItemStack.CODEC.parse(level.registryAccess().createSerializationContext(NbtOps.INSTANCE), compoundTag).result().orElse(ItemStack.EMPTY);
            if (!questBookStack.isEmpty()) {
                questBookStack.set(DataComponents.CUSTOM_NAME, Component.translatable(stopPkBookName).withStyle(ChatFormatting.YELLOW));
