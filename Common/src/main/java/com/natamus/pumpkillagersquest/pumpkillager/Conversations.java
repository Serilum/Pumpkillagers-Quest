...
public class Conversations {
    ...
    public static void startInitialPrisonerShout(Level level, Villager prisoner, Player targetPlayer) { // 3
        if (prisoner.entityTags().contains(Reference.MOD_ID + ".shoutedto." + targetPlayer.getName().getString())) {
            return;
        }
        prisoner.entityTags().add(Reference.MOD_ID + ".shoutedto." + targetPlayer.getName().getString());
    ...
    public static void startPostPrisonerShout(Level level, Villager prisoner, Player targetPlayer) { // 4
        ...
        prisoner.entityTags().add(Reference.MOD_ID + ".shoutedtotwice." + targetPlayerName);
    }
    ...
    public static void startGettingToKnowPrisoner(Level level, Villager prisoner, Player targetPlayer) { // 5
        ...
        prisoner.entityTags().remove(Reference.MOD_ID + ".persistent");
        ...
        addMessage(level, prisoner, targetPlayer, "You'll need this book. It contains ancient knowledge on how to stop him. Read it thoroughly.", ChatFormatting.WHITE, 24010, Data.getStopPkbook(level), "give");
    ...
    public static void startPrisonerGenerationConversation(Level level, Villager prisoner, Player targetPlayer) { // 6
        ...
        bowStack.enchant(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.POWER), 3);
        ...
        targetPlayer.entityTags().add(Reference.MOD_ID + ".cansummonfinalform");
    }
    ...
    public static void startInitialFinalBossConversation(Level level, Villager pumpkillager, Player targetPlayer) { // 7
        ...
        String suffixa = "";
        String suffixb = "";
        if (targetPlayer.entityTags().contains(Reference.MOD_ID + ".diedonce")) {
            suffixa = " AGAIN";
            suffixb = " still";
        }
    ...
    public static void addMessage(Level level, Villager character, Pair<Player, MutableComponent> messagePair, int msDelay, ItemStack itemStack, String itemStackBehaviour) {
        if (character != null) {
            if (character.entityTags().contains(Reference.MOD_ID + ".iskilled")) {
                return;
            }
        }
