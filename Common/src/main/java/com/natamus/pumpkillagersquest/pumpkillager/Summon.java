...
public class Summon {
    public static void summonFinalBossMinions(Level level, Villager pumpkillager, Player targetPlayer, int summonId) {
        ...
        if (pumpkillager.entityTags().contains(Reference.MOD_ID + ".preventactions")) {
            return;
        }
    ...
    public static void summonMinionsAround(Level level, Villager pumpkillager, Player player, BlockPos centerPos, List<LivingEntity> minionsToSpawn) {
        if (pumpkillager.entityTags().contains(Reference.MOD_ID + ".iskilled")) {
            return;
        }
        ...
        for (LivingEntity livingEntity : minionsToSpawn) {
            livingEntity.entityTags().add(Reference.MOD_ID + ".summoned");
            livingEntity.entityTags().add(Reference.MOD_ID + ".justadded");
    ...
    public static void summonFirstWave(Level level, Villager pumpkillager, Player player) {
        ...
        swordStack.enchant(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.VANISHING_CURSE), 1);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            EntityFunctions.getTargetSelector(zombieVillager).addGoal(2, new NearestAttackableTargetGoal<>(zombieVillager, Player.class, false));
        ...
        pumpkillager.entityTags().add(Reference.MOD_ID + ".summoninglightning");
    ...
    public static void summonSecondWave(Level level, Villager pumpkillager, Player player) {
        ...
        bowStack.enchant(level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.VANISHING_CURSE), 1);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            EntityFunctions.getTargetSelector(skeleton).addGoal(2, new NearestAttackableTargetGoal<>(skeleton, Player.class, false));
    ...
    public static void summonFourthWave(Level level, Villager pumpkillager, Player player) {
        ...
        axeStack.enchant(enchantmentRegistry.getOrThrow(Enchantments.VANISHING_CURSE), 1);

        ItemStack crossbowStack = new ItemStack(Items.CROSSBOW, 1);
        crossbowStack.enchant(enchantmentRegistry.getOrThrow(Enchantments.VANISHING_CURSE), 1);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            EntityFunctions.getTargetSelector(pillager).removeAllGoals(goal -> true);

            EntityFunctions.getTargetSelector(vindicator).addGoal(2, new NearestAttackableTargetGoal<>(vindicator, Player.class, false));
            EntityFunctions.getTargetSelector(pillager).addGoal(2, new NearestAttackableTargetGoal<>(pillager, Player.class, true));
