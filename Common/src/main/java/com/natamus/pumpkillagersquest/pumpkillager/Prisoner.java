...
public class Prisoner {
    public static Villager createPrisoner(Level level, BlockPos pos, Player player, VillagerProfession profession, ItemStack headStack, ChatFormatting nameColour, boolean isKnown) {
        ...
        if (!isKnown) {
            prisoner.setCustomName(new TextComponent(Data.prisonerNameUnknown).withStyle(nameColour));
        }
        else {
            prisoner.setCustomName(new TextComponent(Data.prisonerNameKnown).withStyle(nameColour));
        }
