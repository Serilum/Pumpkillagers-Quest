...
public class Prisoner {
    public static Villager createPrisoner(Level level, BlockPos pos, Player player, ResourceKey<VillagerProfession> profession, ItemStack headStack, ChatFormatting nameColour, boolean isKnown) {
        ...
        if (profession == null) {
            ...
            prisoner.setVillagerData(prisoner.getVillagerData().withType(level.registryAccess(), VillagerType.SNOW).withProfession(level.registryAccess(), profession));
        }
        ...
        prisoner.entityTags().add(Reference.MOD_ID + ".prisoner");
        prisoner.entityTags().add(Reference.MOD_ID + ".justadded");
    ...
    public static void checkForSpectralArrows(Level level, Villager pumpkillager, Player player) {
        if (player.entityTags().contains(Reference.MOD_ID + ".extraarrows")) {
            return;
        }
        ...
        player.entityTags().add(Reference.MOD_ID + ".extraarrows");
    }
}
